...
import {
  createChart1DataSets,
  createValidRows,
} from "./xBarChartsHelpFunction";

const XBarChart1 = (props) => {
  const { chartProperties, headings, subgroupSize, tableData, saveChart1Name } =
    props;
  ...
  // Get valid rows from table data
  useEffect(() => {
  ...
  // Create line datasets for charts
  useEffect(() => {
    const dataSet = createChart1DataSets(
      valuesData,
      lockLimit,
      chartProperties,
      xLabels,
  ...
  }, [valuesData, lockLimit, chartProperties]);
